...
def test_fetch_rss_feed_enriches_summary_with_fulltext(monkeypatch):
    ...
    assert items[0]["fulltext_enriched"] is True


def test_fulltext_enrichment_cannot_remove_an_already_relevant_item(monkeypatch):
    feed_config = {
        "name": "Engineering Feed",
        "url": "https://example.com/rss.xml",
        "note_folder": "30-Daily/AI-News",
    ...
    entries = [
        {
            "title": "Agent release",
            "link": "https://example.com/post",
            "id": "g-relevant-before-enrichment",
            "summary": "Agent workflow release.",
            "published_parsed": time.strptime("2026-08-25", "%Y-%m-%d"),
        }
    ]
    scores = iter(((8, ["initial-match"]), (0, ["fulltext-noise"])))
    monkeypatch.setattr(fetcher, "_parse_feed", lambda _url: _feed(entries))
    monkeypatch.setattr(fetcher, "_fetch_article_fulltext", lambda _url: "Long article body")
    monkeypatch.setattr(fetcher, "_score_ai_interest", lambda *_args, **_kwargs: next(scores))
...
def test_fulltext_fetch_rejects_private_targets_and_redirects(monkeypatch):
    calls: list[str] = []
    ...
    monkeypatch.setattr(fetcher.HTTP_SESSION, "get", fake_get)

    assert fetcher._fetch_article_fulltext("http://127.0.0.1/private") == ""
    assert fetcher._fetch_article_fulltext("https://93.184.216.34/article") == ""
...
def test_fetch_youtube_channel_raw_reports_terminal_failure(monkeypatch):
    channel = {"name": "Broken", "channel_id": "id"}
    monkeypatch.setattr(
        fetcher, "_parse_youtube_feed", lambda cid: (_ for _ in ()).throw(RuntimeError("boom"))
    )

    videos, meta = fetcher.fetch_youtube_channel_raw(channel, return_meta=True)

    assert videos == []
    assert meta["status"] == "error"
    assert meta["reason_code"] == "NETWORK_ERROR"
